...
export colorpicker=/home/boban/Scripts/color_dialog.sh
export PATH=$PATH:$colorpicker
...
################################################################################
## Pandoc download webpage .html convert to .md file

function pandocweb () {
	pandoc -s -r html $1 -o $2
}

################################################################################
## SSH prompt color change
## Check https://bbs.archlinux.org/viewtopic.php?id=71715

#if [ -z "$SSH_CONNECTION" ]; then text="ssh"
#fi
#export PS1=' \[\e[1;33m\]\u@\[\e[1;35m\]\h:\w\$\[\e[0;31m\] '
...
wcl () {
	echo -e "Europe - `TZ='Europe/Amsterdam' date +%r`\nAmsterdam - `TZ='Asia/Shanghai' date +%r`\nShanghai - `TZ='Asia/Shenzen' date +%r`\nShenzen - `TZ='America/New_York' date +%r`" | gxmessage -name Timezones -file -
}
